...
class DatabaseTest
{
    ...
    public function testBuildQuery(): void
    {
        ...
        // -- Extra tests to check how the conditional block works ---------- //
//        $results[] = $this->db->buildQuery(
//            'SELECT name FROM users WHERE ?# IN (?a){ AND block = ?d}{ AND active = ?d}',
//            ['user_id', [1, 2, 3], true, true]
//        );
//        $results[] = $this->db->buildQuery(
//            'SELECT name FROM users WHERE ?# IN (?a){ AND block = ?d}{ AND active = ?d}',
//            ['user_id', [1, 2, 3], true, false]
//        );
//        $results[] = $this->db->buildQuery(
//            'SELECT name FROM users WHERE ?# IN (?a){ AND block = ?d{ AND active = ?d}}',
//            ['user_id', [1, 2, 3], true, false]
...
//            'SELECT name FROM users WHERE `user_id` IN (1, 2, 3) AND block = 1 AND active = 1',
//            'SELECT name FROM users WHERE `user_id` IN (1, 2, 3) AND block = 1',
//            'SELECT name FROM users WHERE `user_id` IN (1, 2, 3) AND block = 1',
        ];

        if ($results !== $correct) {
            throw new Exception('Tests are failed!');
        }
    }
}
